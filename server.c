...
void joueurSuivant(){
	...
	if(!joueurPeutJouer[joueurCourant]) joueurSuivant();
}

void annonceJoueurSuivant(){
	joueurSuivant();
	char reply[256];
		...
		else if (fsmServer == 1)
		{
			...
			case 'G':
				...
				if(pId == joueurCourant){
					if(sId == deck[12]){
						// end game 
						sprintf(reply, "E %d", pId);
						broadcastMessage(reply);

						exit(0);
					}
					else{
						// prevent pId from playing again
						joueurPeutJouer[pId] = 0; 

						sprintf(reply, "N %d", sId);
						broadcastMessage(reply);

						// check if only one player can play
						int c = 0, wId = 0;
						for(i = 0; i < 4; i++){
							c += joueurPeutJouer[i];
							wId = joueurPeutJouer[i] ? i : wId; 
						}
						if(c == 1){
							sprintf(reply, "E %d", wId);
							broadcastMessage(reply);
			...
			case 'O':
				...
				if(pId == joueurCourant){
					for(int i = 0; i < 4; i++)
						if(i != pId){
							sprintf(reply, "V %d %d %d", i, oId, tableCartes[i][oId] > 0 ? 100 : 0);
							printf("%d\n", tableCartes[i][oId]);
							broadcastMessage(reply);
						}
			...
			case 'S':
				...
				if(pId == joueurCourant){
					sprintf(reply, "V %d %d %d", tId, oId, tableCartes[tId][oId]);
					broadcastMessage(reply);
